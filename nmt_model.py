...
class NMT(nn.Module):
  ...
  def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    ...
    enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=True)   # ignore warnings. PackedSequence given input. Output of same type
  ...
  def step(self, Ybar_t: torch.Tensor,
    ...
    e_t = torch.bmm(enc_hiddens_proj, dec_state[0].unsqueeze(2)).squeeze(dim=2)  # (src_len, h) * (h, 1) = (src_len)
    ### END YOUR CODE
    ...
    a_t = torch.bmm(enc_hiddens.permute(0, 2, 1), alpha_t.unsqueeze(dim=2)).squeeze(dim=2)  # (b, 2h, src_len) * (b, src_len, 1) = (b, 2h, 1)
